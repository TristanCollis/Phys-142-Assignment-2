...
import constants
from helpers import expectation, H_operator, x_operator
...
def t_tunnel(psi_initial: np.ndarray[complex, Any]) -> float:
    ...
    return float(np.argmin(expectation_x) * constants.EPSILON)
...
def display(data: tuple[np.ndarray[float, Any], np.ndarray[float, Any]], show: bool = False) -> None:
